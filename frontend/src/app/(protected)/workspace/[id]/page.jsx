...
export default function WorkspaceDashboard() {
  ...
  return (
    <div className="flex h-full min-h-0 w-full overflow-hidden bg-[color:var(--background)] text-[color:var(--foreground)] transition-all duration-300 p-0 md:p-3 md:pl-0">
      ...
      {/* Header Banner */}
      <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 bg-gradient-to-r from-card/80 to-muted/40 border border-border rounded-3xl p-6 md:p-8 backdrop-blur-xl animate-list-appear shadow-sm">
        <div className="flex flex-col min-w-0">
          ...
          <h1 className="text-3xl md:text-4xl font-extrabold text-foreground tracking-tight leading-none mb-3">
        ...
        <div className="flex items-center gap-6 self-start md:self-center">
          <div className="flex flex-col items-center bg-card/75 border border-border rounded-2xl px-5 py-3 shadow-sm transition duration-300 hover:scale-105">
            <span className="text-2xl font-black text-foreground">{channels.length}</span>
            <span className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider mt-0.5">Channels</span>
          </div>
          <div className="flex flex-col items-center bg-card/75 border border-border rounded-2xl px-5 py-3 shadow-sm transition duration-300 hover:scale-105">
            <span className="text-2xl font-black text-foreground">{groups.length}</span>
            <span className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider mt-0.5">Groups</span>
          </div>
          <div className="flex flex-col items-center bg-card/75 border border-border rounded-2xl px-5 py-3 shadow-sm transition duration-300 hover:scale-105">
            <span className="text-2xl font-black text-foreground">{dms.length + 1}</span>
            <span className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider mt-0.5">Members</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 relative z-10">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6 animate-list-appear" style={{ animationDelay: "100ms" }}>
          <div className="flex items-center justify-between">
            ...
            {combinedRecents.length > 0 && (
              <span className="text-xs text-muted-foreground font-medium">{combinedRecents.length} active chats</span>
            )}
          </div>

          {combinedRecents.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 border border-dashed border-border rounded-3xl bg-card/40 backdrop-blur-sm">
              <MessageSquare size={32} className="text-muted-foreground mb-3" />
              <p className="text-foreground text-sm font-medium">No recent conversations yet</p>
              <p className="text-muted-foreground text-xs mt-1">Start chatting by choosing a channel or user on the left!</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-4">
              {combinedRecents.slice(0, 5).map((recent) => (
                <div
                  ...
                  className="group relative flex items-center justify-between gap-4 p-4 rounded-2xl border border-border bg-card hover:bg-muted/30 cursor-pointer transition transform hover:scale-[1.01] hover:-translate-y-[1px] active:scale-[0.99] duration-300 shadow-sm"
                >
                  <div className="flex items-center gap-4 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-foreground text-sm group-hover:text-[color:var(--primary)] transition-colors truncate">
                          {recent.name}
                        </span>
                        <span className="px-1.5 py-0.5 rounded text-[8px] font-bold uppercase tracking-wider bg-muted text-muted-foreground">
                          {recent.type}
                        </span>
                      </div>
                      <p className="text-xs text-muted-foreground truncate mt-1">
                        <span className="text-muted-foreground/80 font-semibold">{recent.lastMessage.senderId === recent.id ? recent.name : "You"}: </span>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <ArrowRight size={14} className="text-muted-foreground group-hover:text-[color:var(--primary)] group-hover:translate-x-0.5 transition duration-300" />
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="flex flex-col gap-6 animate-list-appear" style={{ animationDelay: "200ms" }}>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-foreground tracking-tight">Active Team</h2>
          </div>

          <div className="border border-border rounded-3xl bg-card p-5 flex flex-col gap-4 shadow-sm">
            <div className="flex items-center justify-between text-xs font-bold uppercase tracking-wider text-muted-foreground border-b border-border pb-3">
            ...
            {onlineMembers.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8 text-muted-foreground">
                <p className="text-xs text-muted-foreground font-medium">Everyone is away</p>
                <p className="text-[10px] text-muted-foreground/60 mt-0.5">We will keep an eye out for updates!</p>
              </div>
            ) : (
              <div className="flex flex-col gap-3 max-h-80 overflow-y-auto no-scrollbar">
                {onlineMembers.map((member) => (
                  <div
                    ...
                    className="flex items-center justify-between p-2 rounded-xl hover:bg-muted cursor-pointer transition duration-200"
                  >
                    <div className="flex items-center gap-3">
                      <Avatar
                      ...
                      <div className="flex flex-col">
                        <span className="text-xs font-semibold text-foreground truncate max-w-[120px]">{member.fullName || member.name}</span>
                        <span className="text-[9px] text-green-400 font-medium mt-0.5">Available</span>
                      </div>
                    </div>
                    <button className="text-[10px] text-[color:var(--primary)] font-bold hover:text-[color:var(--accent)] transition uppercase tracking-wider">
                      Chat
                    </button>
                  </div>
                ))}
